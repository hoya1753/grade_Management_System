...
<script>
	$(document).ready(function() {
		...
		//수정 버튼 클릭시 해당 과목명ID, 과목명 불러오기
		$("button.modify").on("click", function() {
			$("#id2").val($(this).parents("tr").children().eq(0).text());
			$("#subject2").val($(this).parents("tr").children().eq(1).text());
			$("#admin_basicInfo_subject_modify").modal();

		});

		//삭제 버튼 클릭시 해당 과목명ID, 과목명 불러오기
		$("button.delete").on("click", function() {
			$("#id3").val($(this).parents("tr").children().eq(0).text());
			$("#subject3").val($(this).parents("tr").children().eq(1).text());
			$("#admin_basicInfo_subject_delete").modal();
		});
...
<!-- Navigation 시작 -->
		...
		<div id="page-wrapper">
			...
			<div class="row">
				<div class="col-lg-12">
					<h4 style="padding-top: 10px">기초 정보 관리 > 과목명</h4>
					<hr>
			...
			value="${a.subject_name_id}"
			<c:forEach var="b" items="${subject_list}" > ${b.subject_name_id==a.subject_name_id?"disabled=\"disabled\"":""}</c:forEach>>삭제</button></a></td>
	<!-- 	<td><a href="#"><button class="btn btn-default btn-xs delete">삭제</button></a></td> -->
									</tr>
								</c:forEach>
							</tbody>
						</table>
					...
					<div style="text-align: center;">
						<button type="button" class="btn btn-default addSubject"
							data-toggle="modal" data-target="#admin_basicInfo_subject">과목명
							등록</button>
						<form class="form-inline" method="post">
							<div class="form-group">
								<select class="form-control" name="skey" id="skey">
									<option value="all">전체</option>
									<option value="subject_name_id">과목명ID</option>
							...
							<span class="seach_info">총 <span>${length}</span>개의 과목명 검색
							</span>
						</form>
					</div>
	...
	<%-- 과목명 추가 Modal --%>
	<div id="admin_basicInfo_subject" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">과목명 추가</h4>
				</div>
				<div class="modal-body">
					<%-- action="" 속성의 주소 변경 --%>
					<form action="ad_info_addSubName.it" method="post">
						...
						<div class="form-group">
							<label for="subject">과목명:</label> <input
								class="form-control form-inline" id="subject1" name="subject"
	...
	<%-- 과목명 수정 Modal --%>
	<div id="admin_basicInfo_subject_modify" class="modal fade"
		role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">과목명 수정</h4>
				</div>
				<div class="modal-body">
					<%-- action="" 속성의 주소 변경 --%>
					<form action="ad_info_modifySubName.it" method="post">
						...
						<%-- id="" 속성의 식별자 유일한 이름을 가지도록 수정 --%>
						<div class="form-group">
							<label for="subject">과목명ID:</label> <input
								class="form-control form-inline" id="id2" name="subject_name_id" readonly></input>
						</div>
						<div class="form-group">
							<label for="subject">과목명:</label> <input
								class="form-control form-inline" id="subject2" name="subject"
								maxlength="20" required></input>
						</div>
	...
	<%-- 과목명 삭제 Modal --%>
	<div id="admin_basicInfo_subject_delete" class="modal fade"
		role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">&times;</button>
					<h4 class="modal-title">과목명 삭제</h4>
				</div>
				<div class="modal-body">
					<%-- action="" 속성의 주소 변경 --%>
					<form action="ad_info_deleteSubName.it" method="post">
						...
						<div class="form-group">
							<label for="subject">과목명ID:</label> <input
								class="form-control form-inline" id="id3" name="subject_name_id" readonly></input>
						</div>
						<div class="form-group">
							<label for="subject">과목명:</label> <input
								class="form-control form-inline" id="subject3" name="subject"
								maxlength="20" readonly></input>
						</div>
						<div class="form-group">현재 과목명 정보를 삭제할까요?</div>

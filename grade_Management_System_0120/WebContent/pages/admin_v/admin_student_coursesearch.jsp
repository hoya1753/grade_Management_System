...
<script>
	$(document).ready(function() {
		...
		$("button.choose").on("click", function() {
			...
			title = $(this).parents("tr").children().eq(1).text();
			title_date = $(this).parents("tr").children().eq(2).text();

			$("#title").text(title);
			$("#title_date").text(title_date);
			course_id = $(this).parents("tr").children().eq(0).text();

			/* 과정에 속한 학생정보 불러오기 */
			$.post("admin_student_coursesearch_ajax.it", {
				course_id : course_id
			}, function(data) {
				...
				for (var i = 0; i < obj_length; i++) {
					var obj = myObj.student[i];
					result += "<tr>";
					result += "<td>" + obj.student_id + "</td>";
					result += "<td>" + obj.st_name + "</td>";
					result += "<td>" + obj.st_phone + "</td>";
					result += "<td>" + obj.st_registration_date + "</td>";
					result += "<td>" + obj.fail_date + "</td>";
					result += "</tr>";
				}
				if ($.trim(data) != "") {
			...
			$("#admin_student_choosemodal").modal();
		});
		...
		$("button.modalsearch").on("click", function() {
			...
			$.post("admin_student_coursesearch_ajax.it", {
				...
				for (var i = 0; i < obj_length; i++) {
					...
					result += "<td>" + obj.student_id + "</td>";
					result += "<td>" + obj.st_name + "</td>";
					result += "<td>" + obj.st_social_num + "</td>";
					result += "<td>" + obj.st_phone + "</td>";
					result += "<td>" + obj.st_registration_date + "</td>";
					result += "<td>" + obj.fail_date + "</td>";
					result += "</tr>";
				...
				//select가 바뀔 때 검색 입력 값은 초기화 되도록 지정
				$("#skey1").on("change", function() {
					$("#svalue1").val("");
				});
				//검색 진행후 검색 키워드가 화면에 그대로 남아있도록 액션 추가
				//-> 검색 폼 부분에 기존 검색 키워드를 재설정
				$("#skey1").val(skey).attr("selected", "selected");
				$("#svalue1").val(svalue);
...
<body>
	...
	<!-- Navigation 시작 -->
		...
		<div id="page-wrapper">
			...
			<div class="row">
				<div class="col-lg-12">
					<h4 style="padding-top: 10px">학생 관리 > 과정검색</h4>
					<hr>
					<div class="table-responsive list">
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								...
								<%-- <AdminAction5) ad_st_allCourseList --%>
								<c:forEach var="ad" items="${list}">
									<tr>
										...
										<td><button type="button"
												class="btn btn-default btn-xs choose">보기</button></td>
									</tr>
								</c:forEach>
	...
	<%-- Modal --%>
<%-- 상세정보 Modal --%>
<div id="admin_student_choosemodal" class="modal fade" role="dialog">
	...
	<div class="modal-content">
		...
		<div class="modal-body">
			...
			<p style="padding: 1%; text-align: right;">
				총 <span id="st_count"></span>명의 수강생 정보 검색
			</p>
				<!-- 검색 폼 -->
				<div style="text-align: center;">
					<form class="form-inline">
						<div class="form-group">
							<select class="form-control" name="skey" id="skey1">
								<option value="all">전체</option>
								<option value="student_id">수강생ID</option>
								<option value="name">이름</option>
								<!-- 검색 기준 추가 -->
							</select> <input type="text" class="form-control" id="svalue1"
								name="svalue" style="width: 180px">
							<!-- svalue 랑 svalue매칭 시키는 액션 필요 -->
